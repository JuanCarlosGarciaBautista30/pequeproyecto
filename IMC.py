...
while True:
    ...
    if (not re.fullmatch(r'[A-Za-z ñ ó ú í é á ü Ü ]{3,25}',apellido_paterno)):
      
       print('valor invalido. ( Campo vacio, numeros y simbolos estan prohibidos para este campo), intenta otravez.')
       continue
    
    # Usamos un else y su braak para dejar de iterar.
    else:
      break
...
while True:
    ...
    apellido_materno = input('Apellido Materno: ') # pedimos el apellido materno.

    if (not re.fullmatch(r'[A-Za-z ñ ó ú í é á ü Ü  ]{3,25}',apellido_materno)):
  ...
  # Usamos nuevamente un while true.
while True:
    try: # con el bloque Try intentaremos ejecutar las lineas que queremos ejecutar.
        edad = int(input('Edad: ')) # Pedimos la edad pero devera ser de tipo entero. 
...
while True:
    try:
        peso = float(input('peso (kg): ')) # pedimos el peso del usuario con una variabl de tipo float, pues el peso no necesariamente es un entero
        
        if peso < 0: # Un peso siempre sera positivo.
